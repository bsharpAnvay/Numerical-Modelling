...
function F = matrixEquations(vars,t,dx,dy,dt,theta,l,old_phi)
a=(l/dx)+1;              % number of points in a row
b=(l/dy)+1;              % number of points in a column
xsq=(dx)^2;
ysq=(dy)^2;
...
for i=1:b
    j=1;
    phi(i,j)=5+5*sin(pi/2+t*dt);
end
phi(2:b-1,2:a-1) = reshape(vars, [b-2, a-2]);  % Reshape vars into a (b-2)x(a-2) matrix
F = zeros((a-2)*(b-2), 1);            % initialising F
for i =2:b-1
    for j = 2:a-1
        F((a-2)*(i-2)+j-1) = (phi(i,j)- old_phi(i,j))/dt -theta*(((1+phi(i,j+1)+phi(i,j+1)^2)*(phi(i,j+1))^2-2*(1+phi(i,j)+phi(i,j)^2)*(phi(i,j))^2 + (1+phi(i,j-1)+phi(i,j-1)^2)*(phi(i,j-1))^2)/xsq + ((1+phi(i+1,j)+phi(i+1,j)^2)*(phi(i+1,j))^2-2*(1+phi(i,j)+phi(i,j)^2)*(phi(i,j))^2 + (1+phi(i-1,j)+phi(i-1,j)^2)*(phi(i-1,j))^2)/ysq) + (theta-1)*(((1+phi(i,j+1)+phi(i,j+1)^2)*(old_phi(i,j+1))^2-2*(1+phi(i,j)+phi(i,j)^2)*(old_phi(i,j))^2 + (1+phi(i,j-1)+phi(i,j-1)^2)*(old_phi(i,j-1))^2)/xsq + ((1+phi(i+1,j)+phi(i+1,j)^2)*(old_phi(i+1,j))^2-2*(1+phi(i,j)+phi(i,j)^2)*(old_phi(i,j))^2 + (1+phi(i-1,j)+phi(i-1,j)^2)*(old_phi(i-1,j))^2)/ysq);
    end
end
end
...
for t=1:10
    ...
    clf;
    [sol, fval, exitflag] = fsolve(@(vars)matrixEquations(vars,t,dx,dy,dt,theta,l,old_phi), initialGuess, options);
    old_phi(2:b-1,2:a-1) = reshape(sol, [b-2, a-2]);
    for i=1:b
        j=1;
        old_phi(i,j)=5+5*sin(pi/2+t*dt);
    end
end
phi=old_phi;
